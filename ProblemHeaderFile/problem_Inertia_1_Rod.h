...
#include "calculate_Inertia_1_Rod.h"
...
//7.1
void Inertia_1_Rod(SDL_Window* window, SDL_Renderer* render, SDL_Texture* img[],SDL_Rect img_rect[],int &mouse_x,int &mouse_y){
    ...
    //Input surface
    rod.font= TTF_OpenFont("Ubuntu/Ubuntu-Bold.ttf",26);
    ...
    while(run){
        ...
        surface = TTF_RenderText_Solid(rod.font, " BACK ", color);
        ...
        SDL_RenderCopy(render,img[32],NULL,&rect[2]);

        //image rod_end
        rect[3]={650,10,350,300};
        SDL_RenderCopy(render,img[33],NULL,&rect[3]);

        //type
        surface=TTF_RenderText_Solid(rod.font," Rod ",color2);
        tex=SDL_CreateTextureFromSurface(render,surface);
        rect[4]={70,330,70,40};
        SDL_FreeSurface(surface);
        ...
        //option 1
        surface=TTF_RenderText_Solid(rod.font,"1. Inertia about Center axis ",color1);
        tex=SDL_CreateTextureFromSurface(render,surface);
        ...
        //option 2
        surface=TTF_RenderText_Solid(rod.font,"2. Inertia about End axis ",color1);
        tex=SDL_CreateTextureFromSurface(render,surface);
        ...
        //options
        surface=TTF_RenderText_Solid(rod.font,"  1 / 2  ? ",color1);
        tex=SDL_CreateTextureFromSurface(render,surface);
        ...
        //mass
        surface=TTF_RenderText_Solid(rod.font," m - mass of the rod (kg): ",color);
        tex=SDL_CreateTextureFromSurface(render,surface);
        ...
        //length
        surface= TTF_RenderText_Solid(rod.font," l - length of the rod (m): ",color);
        tex=SDL_CreateTextureFromSurface(render,surface);
        ...
        surface = TTF_RenderText_Solid(rod.font, " CALCULATE ", color);
        tex = SDL_CreateTextureFromSurface(render, surface);
                    ...
                    switch(e.button.button){
                        ...
                        if((mouse_x>=0&&mouse_x<=100)&&(mouse_y>=0&&mouse_y<=50)){
                            ...
                            SDL_ShowSimpleMessageBox(10,"Invalid","Please fill Every box properly!",window);
                                ...
                                Cal_Inertia_1_Rod(window,render,img,img_rect,st1,st2,st3,mouse_x,mouse_y);
                                st1.clear();st2.clear();st3.clear();
                            }
                        }
                        break;
        ...
        //input 1
        if ( st1.size() ) {
            surface = TTF_RenderText_Solid(rod.font, st1.c_str(), color);
            tex = SDL_CreateTextureFromSurface(render, surface);
        ...
        //input 2
        if ( st2.size() ) {
            surface = TTF_RenderText_Solid(rod.font, st2.c_str(), color);
            tex = SDL_CreateTextureFromSurface(render, surface);
        ...
        //input 3
        if ( st3.size() ) {
            surface = TTF_RenderText_Solid(rod.font, st3.c_str(), color);
            tex = SDL_CreateTextureFromSurface(render, surface);
