...
#include "calculate_Surface_Tension.h"
...
//4
void Surface_Tension(SDL_Window* window, SDL_Renderer* render, SDL_Texture* img[],SDL_Rect img_rect[],int &mouse_x,int &mouse_y){
    ...
    //Input surface
    ST.font= TTF_OpenFont("Ubuntu/Ubuntu-Bold.ttf",26);
    ...
    while(run){
        ...
        //image
        rect[7]={300,10,600,330};
        SDL_RenderCopy(render,img[16],NULL,&rect[7]);

		//for values
        surface= TTF_RenderText_Solid(ST.font," Give the values of - ",color1);
        ...
        //n
        surface=TTF_RenderText_Solid(ST.font," n - how many bubbles : ",color);
        tex=SDL_CreateTextureFromSurface(render,surface);
        rect[2]={30,450,220,30};
        SDL_FreeSurface(surface);
        ...
        //r-radius
        surface=TTF_RenderText_Solid(ST.font," r- radius of the small bubbles (m): ",color);
        tex=SDL_CreateTextureFromSurface(render,surface);
        ...
        //R-Radius
        surface= TTF_RenderText_Solid(ST.font," R- Radius of the large bubble (m):  ",color);
        tex=SDL_CreateTextureFromSurface(render,surface);
        rect[0]={30,530,300,30};
        SDL_FreeSurface(surface);
        ...
        //T-Surface Tension
        surface=TTF_RenderText_Solid(ST.font," T- Surface Tension (N/m): ",color);
        tex=SDL_CreateTextureFromSurface(render,surface);
        rect[1]={30,570,250,30};
        SDL_FreeSurface(surface);
        ...
        surface = TTF_RenderText_Solid(ST.font, " CALCULATE ", color);
        tex = SDL_CreateTextureFromSurface(render, surface);
        ...
        surface = TTF_RenderText_Solid(ST.font, " BACK ", color);
        tex = SDL_CreateTextureFromSurface(render, surface);
                    ...
                    switch(e.button.button){
                        ...
                        if((mouse_x>=0&&mouse_x<=100)&&(mouse_y>=0&&mouse_y<=50)){
                            ...
                            SDL_ShowSimpleMessageBox(10,"Invalid","Please fill Every box properly!",window);
                                ...
                                Cal_Surface_Tension(window,render,img,img_rect,st1,st2,st3,st4,mouse_x,mouse_y);
                                st1.clear();st2.clear();st3.clear();st4.clear();
                            }
                        }
                        break;
        ...
        //input 1
        if ( st1.size() ) {
            surface = TTF_RenderText_Solid(ST.font, st1.c_str(), color);
            tex = SDL_CreateTextureFromSurface(render, surface);
        ...
        //input 2
        if ( st2.size() ) {
            surface = TTF_RenderText_Solid(ST.font, st2.c_str(), color);
            tex = SDL_CreateTextureFromSurface(render, surface);
        ...
        //input 3
        if ( st3.size() ) {
            surface = TTF_RenderText_Solid(ST.font, st3.c_str(), color);
            tex = SDL_CreateTextureFromSurface(render, surface);
        ...
        //input 4
        if ( st4.size() ) {
            surface = TTF_RenderText_Solid(ST.font, st4.c_str(), color);
            tex = SDL_CreateTextureFromSurface(render, surface);
